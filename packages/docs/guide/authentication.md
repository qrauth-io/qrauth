...
The QR-based **AuthSession** "Sign in with QRAuth" flow documented here is
**frozen for new public integrations**. New sign-in integrations should use
the [OIDC provider](/guide/oidc) — QRAuth's certified, standards-based sign-in
surface. Existing integrations keep working. (This applies to QR-based sign-in
only; API-key and management-API auth are unaffected.)
:::
