...
The `<qrauth-login>` component drives the QR-based **AuthSession** sign-in
flow, which is **frozen for new public integrations** — new sign-in
integrations should use the [OIDC provider](/guide/oidc), QRAuth's certified,
standards-based sign-in surface. The `<qrauth-2fa>` and `<qrauth-ephemeral>`
components are not sign-in and are unaffected. Existing integrations keep
working.
:::
...
Pin a major.minor in production (`components-0.4.1.js` + SRI). The package follows semver — minor bumps add attributes and event detail fields without breaking; majors remove or rename. Watch the [CHANGELOG](https://github.com/qrauth-io/qrauth/blob/main/packages/web-components/CHANGELOG.md) for upgrade notes.
