...
import { signRsaJws } from './rsa-jws.js';
...
/**
 * ADR-0003 Slice 3a: prefix-free ECDSA JWS sign endpoint for OIDC ID
 * tokens. Additive sibling of `/v1/sign-ecdsa` above — it does NOT change
 * that endpoint's behavior.
 *
 * Unlike `/v1/sign-ecdsa`, this endpoint signs the caller's canonical
 * input bytes DIRECTLY (no `qrauth:ecdsa-canonical:v1:` prefix) and
 * returns the IEEE P1363 raw `R||S` signature, base64url-encoded — the
 * exact shape a stock OIDC client (`jose.jwtVerify`) verifies as the
 * third segment of a compact JWS.
 *
 * Cryptographic-domain separation between the two endpoints holds by
 * input format: the prefix is not valid base64url, so a prefixed-canonical
 * signature can never be a valid JWS and vice versa. The JWS-shape regex
 * (`<base64url>.<base64url>`) is mandatory defense-in-depth — it prevents
 * this prefix-free path from being abused to sign arbitrary bytes. Same
 * key material and same `keyId` → same key as `/v1/sign-ecdsa`; only the
 * input the signer signs differs.
 */
interface SignEcdsaJwsBody {
  keyId: string;
  canonicalInput: string; // JWS signing input: base64url(header).base64url(payload)
}

app.post<{ Body: SignEcdsaJwsBody }>('/v1/sign-ecdsa-jws', async (request, reply) => {
  const { keyId, canonicalInput } = request.body ?? ({} as SignEcdsaJwsBody);
  ...
  request.log.info(
    {
      keyId,
      method: 'jws',
      messageFingerprint: createHash('sha3-256').update(canonicalInput, 'utf8').digest('hex'),
      messageBytes: Buffer.byteLength(canonicalInput, 'utf8'),
      alg: 'ES256',
    },
    'signed-ecdsa-jws',
  );
...
/**
 * ADR-0003 Slice 7a: prefix-free RSA JWS sign endpoint for OIDC ID tokens
 * (RS256 — mandatory-to-implement per OIDC Core §15.1). Additive sibling of
 * `/v1/sign-ecdsa-jws`; it does NOT change any existing endpoint.
 *
 * Signs the caller's canonical input bytes DIRECTLY (no domain prefix) with
 * RSASSA-PKCS1-v1_5 + SHA-256 (RFC 7518 §3.3) and returns the base64url
 * signature — the third segment of a compact JWS a stock OIDC client verifies.
 *
 * Same cryptographic-domain separation as the ECDSA JWS path: the shared
 * `base64url.base64url` shape guard makes the `qrauth:ecdsa-canonical:v1:`
 * prefix (not valid base64url) unreachable here, so this prefix-free path
 * cannot be abused to sign arbitrary bytes. RS256 ≠ ES256: the alg header
 * pins which verifier applies, so the two never cross-verify. The RSA key is
 * looked up by the same `keyId` scheme as ECDSA, from the `.rsa.enc` envelope.
 */
interface SignRsaJwsBody {
  keyId: string;
  canonicalInput: string; // JWS signing input: base64url(header).base64url(payload)
}

app.post<{ Body: SignRsaJwsBody }>('/v1/sign-rsa-jws', async (request, reply) => {
  const { keyId, canonicalInput } = request.body ?? ({} as SignRsaJwsBody);
  if (typeof keyId !== 'string' || typeof canonicalInput !== 'string') {
    return reply.status(400).send({ error: 'malformed_request' });
  }
  ...
  let pem: string;
  try {
    pem = await loadRsaPem(keyId);
  } catch (err) {
    if (err instanceof SignerKeyError) {
      return reply.status(err.status).send({ error: err.code });
    }
    request.log.error({ err }, 'unexpected rsa key load failure');
    return reply.status(500).send({ error: 'internal_error' });
  }
...
interface ProvisionBody {
  ecdsa?: string;
  slhdsa?: string;
  rsa?: string; // ADR-0003 Slice 7b: RSA envelope for OIDC RS256 ID tokens
}
...
app.post<{ Params: { keyId: string }; Body: ProvisionBody }>(
  ...
  async (request, reply) => {
    ...
    const { ecdsa, slhdsa, rsa } = request.body ?? ({} as ProvisionBody);
    if (typeof ecdsa !== 'string' && typeof slhdsa !== 'string' && typeof rsa !== 'string') {
      return reply.status(400).send({ error: 'malformed_request', message: 'at least one of ecdsa, slhdsa or rsa must be provided' });
    }

    const provisioned: string[] = [];

    for (const [kind, envelope] of [['ecdsa', ecdsa], ['slhdsa', slhdsa], ['rsa', rsa]] as const) {
      ...
      const ext = kind === 'ecdsa' ? 'ecdsa.enc' : kind === 'slhdsa' ? 'slhdsa.enc' : 'rsa.enc';
