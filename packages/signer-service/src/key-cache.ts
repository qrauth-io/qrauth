...
interface KeyCaches {
  ...
  rsaKeyCache: LRUCache<string, string>;
  ...
  loadRsaPem(keyId: string): Promise<string>;
...
export function createKeyCaches(keysDir: string): KeyCaches {
  ...
  // ADR-0003 Slice 7a: RSA PEM cache, byte-identical posture to ecdsaKeyCache.
  const rsaKeyCache = new LRUCache<string, string>({
    max: CACHE_MAX,
    ttl: CACHE_TTL_MS,
    ttlAutopurge: false,
    dispose: (_value, _key, reason) => {
      if (reason === 'evict') rsaStats.evictionsLru += 1;
      else if (reason === 'expire') rsaStats.evictionsTtl += 1;
    },
  });
  ...
  // ADR-0003 Slice 7a: RSA PEM loader. Mirrors loadEcdsaPem exactly; only the
  // on-disk envelope extension differs (`.rsa.enc` vs `.ecdsa.enc`). RSA and
  // ECDSA private keys are both PEM, so the decrypt + validate path is shared
  // in shape. Additive — loadEcdsaPem / loadPrivateKey are untouched.
  async function loadRsaPem(keyId: string): Promise<string> {
    const cached = rsaKeyCache.get(keyId);
    if (cached !== undefined) {
      rsaStats.hits += 1;
      return cached;
    }
    ...
    const encPath = join(keysDir, `${keyId}.rsa.enc`);
    let envelope: string;
    try {
      envelope = await readFile(encPath, 'utf8');
    } catch {
  ...
  function snapshot(): CachesSnapshot {
    return {
      caches: {
        ...
        rsa: {
          entries: rsaKeyCache.size,
          hits: rsaStats.hits,
          misses: rsaStats.misses,
          evictions: {
            ttl: rsaStats.evictionsTtl,
            lru: rsaStats.evictionsLru,
          },
        },
      },
