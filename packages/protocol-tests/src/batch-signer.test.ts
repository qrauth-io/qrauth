...
beforeAll(async () => {
  ...
  // AUDIT-FINDING-016: production writes encrypted envelopes under
  // `${keyId}.slhdsa.enc`. Match that here so `loadSlhDsaKeyPair` reads
  // a valid envelope.
  writeFileSync(
...
describe("BatchSigner concurrency", () => {
  ...
  it("the resulting batch root signature verifies against the SLH-DSA pubkey", async () => {
    ...
    // AUDIT-FINDING-010 / AUDIT-2 N-7: `LocalSlhDsaSigner` prepends
    // `qrauth:merkle-root:v1:` before handing bytes to SLH-DSA, so the
    // verifier must reconstruct the same prefixed input. Verifying against
    // the raw merkle root (pre-Finding-010 shape) is the exact regression
    // N-7 is guarding against.
    ...
    expect(await slhDsaVerify(pub, prefixedRoot, sig)).toBe(true);
  });
