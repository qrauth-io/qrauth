...
/**
 * AUDIT-FINDING-010 / AUDIT-2 N-7: both signer backends prepend
 * `qrauth:merkle-root:v1:` before handing bytes to SLH-DSA. Verifiers
 * must reconstruct the same prefixed input. Pinning the literal here
 * lets the suite catch any drift in either the Local or Http backend.
 */
const MERKLE_ROOT_PREFIX = Buffer.from("qrauth:merkle-root:v1:", "utf8");

function prefixed(message: Buffer): Buffer {
  return Buffer.concat([MERKLE_ROOT_PREFIX, message]);
}
...
beforeAll(async () => {
  ...
  // AUDIT-FINDING-016: production reads `${keyId}.slhdsa.enc` encrypted
  // envelopes. Mirror that on disk so `loadSlhDsaKeyPair` can decrypt.
  writeFileSync(
...
describe("LocalSlhDsaSigner", () => {
  it("signs a message that verifies against the loaded public key", async () => {
    ...
    expect(await slhDsaVerify(publicKey, prefixed(message), sig)).toBe(true);
  });
