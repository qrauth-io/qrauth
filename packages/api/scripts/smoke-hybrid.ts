...
import { randomBytes as smokeRandomBytes } from "node:crypto";
...
async function main() {
  ...
  try {
    ...
    // Step 2b: compute the MAC fast-path tag over the unified canonical core.
    const smokeDestHash = await computeDestHash("url", destinationUrl, "");
    const smokeGeoHash = await canonicalGeoHash(lat, lng, radiusM);
    const macCanonical = canonicalizeCore({
      algVersion: result.algVersion,
      token,
      tenantId: org.id,
      destHash: smokeDestHash,
      geoHash: smokeGeoHash,
      expiresAt: "",
    });
    ...
    const ecdsaValid = signingService.verifyCanonical(
    ...
    log(`  legacy-ecdsa → ${checkAlgVersion("ecdsa-p256-sha256-v1")} (expected: unknown after AUDIT-FINDING-011 drop)`);
    log(`  unknown-v9   → ${checkAlgVersion("unknown-v9")} (expected: unknown)`);
    if (checkAlgVersion("ecdsa-p256-sha256-v1") !== "unknown") {
      throw new Error("legacy ecdsa should classify as unknown after canonical unification");
    }
    ...
    const stuckDestHash = await computeDestHash(
      "url",
      `https://example.com/stuck/${stuckToken}`,
      "",
    );
    const stuckGeoHash = await canonicalGeoHash(null, null, null);
    const stuckCanonical = canonicalizeCore({
      algVersion: ALG_VERSION_POLICY.hybrid,
      token: stuckToken,
      tenantId: org.id,
      destHash: stuckDestHash,
      geoHash: stuckGeoHash,
      expiresAt: "",
    });
    const stuckSig = await signingService.signCanonical(signingKey.keyId, stuckCanonical);
    ...
    const bridgeKp = mlDsaKeyPairFromSeed(smokeRandomBytes(MLDSA_LENGTHS.seed));
