...
async function main(): Promise<void> {
  ...
  try {
    ...
    // T4-A: createKeyPair now THROWS if the signer push fails, so an
    // unreachable/broken signer aborts here instead of leaving a DB row the
    // signer can't sign with. The sign+verify probe below stays as a stronger
    // end-to-end check (it proves the signer can actually sign with the key,
    // not merely that the key push returned 2xx).
    const key = await signingService.createKeyPair(org.id);

    // Verify end-to-end that the configured signer can sign with the new key,
    ...
    // Release the BullMQ/Redis connections opened transitively at import time:
    // SigningService → security-webhook → lib/queue eagerly constructs the
    // shared queues, each with its own ioredis connection. prisma.$disconnect()
    // alone leaves those open, which kept this script alive after success
    // (required a manual pkill on prod 2026-05-31). closeQueues() releases them
    // so the process exits on its own. T4-A.
    await closeQueues();
  }
}
