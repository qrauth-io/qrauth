...
export class FraudDetectionService {
  constructor(
    ...
    // Nullable: read-only consumers (e.g. the verify route's quick trust-score
    // lookup) never reach the alerting paths, so they pass `null` rather than
    // wiring an AlertService. The two send sites below guard on it.
  ...
  async analyzeScan(input: ScanAnalysisInput): Promise<ScanAnalysisResult> {
    ...
    for (const incident of createdIncidents) {
      if (this.alertService && alertableSeverities.includes(incident.severity)) {
