...
import { isSafeWebhookUrl } from '../lib/url-validation.js';
...
export class AppService {
  ...
  async createApp(organizationId: string, data: {
    ...
    if (data.webhookUrl) {
      const urlCheck = isSafeWebhookUrl(data.webhookUrl);
      if (!urlCheck.safe) {
  ...
  /**
   * Look up an app by clientId only (no secret verification).
   * Used for PKCE public-client flows where the secret is not required.
   */
  async findByClientId(clientId: string) {
    const app = await this.prisma.app.findUnique({
      where: { clientId },
      select: { id: true, organizationId: true, allowedScopes: true, status: true },
    });
    if (!app || app.status !== 'ACTIVE') return null;
    return { id: app.id, organizationId: app.organizationId, allowedScopes: app.allowedScopes };
  }
}
