...
/**
 * Thrown by signer-authoritative paths when the signer is unavailable and
 * there is no local fallback (ADR-0001 A4-M2 Phase 3). Routes translate
 * this into a 503 — animated-QR now hard-depends on signer reachability.
 */
export class MacSignerUnavailableError extends Error {
  constructor(
    readonly operation: 'register' | 'verify',
    readonly reason: string,
  ) {
    super(`MAC signer unavailable during ${operation} (${reason})`);
    this.name = 'MacSignerUnavailableError';
  }
}
...
export class AnimatedQRService {
  ...
  /**
   * Phase 1 (dual-mode) shadow verify. Runs AFTER local validation has
   * already decided the response. Records frames_observed and
   * divergence into the stats collector; transport failures are
   * accounted separately and do NOT muddy the divergence ratio.
   *
   * **Dual mode only.** In `signer` mode (Phase 2) the route uses
   * `signerVerifyFrame` as the primary path instead, so this comparator
   * never runs.
   *
  ...
  // ===========================================================================
  // ADR-0001 A4-M2 Phase 3 — signer-authoritative paths (no local fallback).
  //
  // Active only when `backend === 'signer'`. Routes dispatch by backend; in
  // signer mode they call `registerWithSignerBlocking` + `signerVerifyFrame`
  // instead of the dual-mode fire-and-forget pair above. As of Phase 3 the
  // local fallback is decommissioned: the signer is the SOLE MAC verifier.
  // Register failure throws; verify failure hard-fails to `valid: false`.
  // Signer reachability is now a hard runtime dependency for animated QR.
  // ===========================================================================

  /**
   * Phase 3: blocking signer registration. Waits for the signer to
   * acknowledge the session. On failure, **throws** — there is no local
   * fallback anymore, so a session the signer does not hold could never
   * be verified. The caller (route) translates the throw into a 503 and
   * does not return a frame secret.
   *
   * Resolves (void) if the signer acknowledged (including idempotent
   * `session_exists`); throws `MacSignerUnavailableError` otherwise.
   */
  async registerWithSignerBlocking(
    ...
    try {
      const clampedTtl = Math.max(60, Math.min(3600, Math.floor(ttlSeconds)));
      result = await this.macSigner.registerSession({
        sessionId: authSessionId,
        binding: AnimatedQRService.sessionBinding(authSessionId),
        ttlSeconds: clampedTtl,
      });
      ...
      this.macSignerStats?.recordRegister('transport');
      this.logger.error(
        { event: 'mac_signer_register_blocking_exception', authSessionId, err },
        'Phase 3 signer register threw — no local fallback, failing session creation',
      );
      throw new MacSignerUnavailableError('register', 'exception');
    }
    ...
    // session_exists is acceptable — idempotent replay; the signer
    // already holds the session under the same binding.
    if (result.reason === 'session_exists') {
      this.macSignerStats?.recordRegister('conflict');
      return;
    }

    this.macSignerStats?.recordRegister(
      result.reason === 'registry_full' ? 'full' : 'transport',
    );
    this.logger.error(
      {
        event: 'mac_signer_register_blocking_failure',
        authSessionId,
        reason: result.reason,
      },
      `Phase 3 signer register failed (${result.reason}) — no local fallback, failing session creation`,
    );
    throw new MacSignerUnavailableError('register', result.reason);
  }

  /**
   * Phase 3: verify a frame via the signer's /mac/verify endpoint.
   *
   * The signer is the SOLE verifier — there is no local fallback. The
   * return `source` is always `'signer'`; it is retained for the route's
   * observability log only and never leaks into the client response
   * (clients see the same `{ valid, reason }` shape).
   *
   * On signer unavailability (transport / circuit-open / session-not-found
   * / exception) this **hard-fails** to `{ valid: false }` with reason
   * `'MAC verification unavailable'` and records a `hard_fail` — a
   * page-able signal. It does NOT fall back to local HKDF validation.
   *
   * Timestamp-freshness checks still run locally before the signer call
   * to avoid spending a network round-trip on obviously-stale frames.
   */
  async signerVerifyFrame(
    baseUrl: string,
    frameIndex: number,
    timestamp: number,
    hmac: string,
    sessionIdentifier: string,
  ): Promise<{ valid: boolean; reason?: string; source: 'signer' }> {
    // 1. Timestamp freshness check — always local, no RPC needed.
    const now = Date.now();
    const age = now - timestamp;
    if (age < -2000) {
      return { valid: false, reason: 'Frame timestamp is in the future', source: 'signer' };
    }
    if (age > 5000) {
      return { valid: false, reason: 'Frame expired (older than 5 seconds)', source: 'signer' };
    }

    // 2. Verify via the signer. This is the only path — no local fallback.
    const payload = Buffer.from(`${baseUrl}:${timestamp}:${frameIndex}`, 'utf8');
    try {
      const result = await this.macSigner.verify({
      ...
      if (result.ok) {
        this.macSignerStats?.recordVerify(result.valid ? 'ok_valid' : 'ok_invalid');
        this.macSignerStats?.recordSignerVerify('primary_ok');
        return {
          valid: result.valid,
          reason: result.valid ? undefined : 'Invalid HMAC',
          source: 'signer',
        };
      }

      // Signer unavailable — hard fail. No local fallback (Phase 3).
      this.macSignerStats?.recordVerify(
        result.reason === 'session_not_found' ? 'session_expired' : 'transport',
      );
      this.macSignerStats?.recordSignerVerify('hard_fail');
      this.logger.error(
        {
          event: 'mac_signer_verify_hard_fail',
          reason: result.reason,
          sessionId: sessionIdentifier,
        },
        `MAC signer verify failed (${result.reason}) — no local fallback, frame rejected`,
      );
      return { valid: false, reason: 'MAC verification unavailable', source: 'signer' };
    } catch (err) {
      this.macSignerStats?.recordVerify('transport');
      this.macSignerStats?.recordSignerVerify('hard_fail');
      this.logger.error(
        { event: 'mac_signer_verify_hard_fail', err, sessionId: sessionIdentifier },
        'MAC signer verify threw — no local fallback, frame rejected',
      );
      return { valid: false, reason: 'MAC verification unavailable', source: 'signer' };
    }
  }
