...
export interface MacSignerStats {
  ...
  /**
   * Signer-authoritative verify observability (ADR-0001 A4-M2 Phase 3).
   *
   * Only meaningful when `backend === 'signer'`. Counts how often the
   * signer was authoritative for a frame verify (`primary_ok`) vs how
   * often the signer was unavailable (`hard_fail`). As of Phase 3 the
   * local fallback is decommissioned: a `hard_fail` is no longer
   * silently covered by local validation — it is a verification outage
   * that surfaces to the client as `valid: false`. Any non-zero
   * `hard_fail` is therefore a page-able signal that the signer is down.
   */
  signer_verify: {
    primary_ok: number;
    hard_fail: number;
...
export type SignerVerifyOutcome = 'primary_ok' | 'hard_fail';
...
export interface MacSignerStatsCollector {
  ...
  recordSignerVerify(outcome: SignerVerifyOutcome): void;
...
export function createMacSignerStatsCollector(args: {
  ...
  const signerVerify = { primary_ok: 0, hard_fail: 0 };
  ...
  return {
    ...
    recordSignerVerify(outcome): void {
      if (outcome === 'primary_ok') {
        signerVerify.primary_ok += 1;
      } else {
        signerVerify.hard_fail += 1;
      }
    },
