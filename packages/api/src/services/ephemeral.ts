...
export class EphemeralSessionService {
   ...
   */
  async createSession(
    owner: { organizationId: string; appId?: string | null },
    data: {
      scopes: string[];
      ttl?: string;
      maxUses?: number;
  ...
  /**
   * Revoke an ephemeral session. Scoped to the owning organization — only a
   * caller authenticated for that org (app credentials or API key/JWT) can
   * revoke it.
   */
  async revokeSession(sessionId: string, organizationId: string) {
    const session = await this.prisma.ephemeralSession.findUnique({
      where: { id: sessionId },
      select: { organizationId: true, token: true, status: true },
    });
    ...
    // Atomic update
    const { count } = await this.prisma.ephemeralSession.updateMany({
      where: { id: sessionId, organizationId, status: { not: 'REVOKED' } },
  ...
  async listSessions(
    ...
    // Auto-expire stale PENDING sessions for this org
