...
export interface EcdsaSigner {
   ...
   * with the ECDSA-P256 private key identified by `keyId`. Returns the
   * base64url-encoded raw `R||S` signature (IEEE P1363, 64 bytes for
   * ES256), suitable for direct concatenation as the third segment of a
   * compact JWS.
   *
   * Unlike `signCanonical`, this method signs the bytes directly WITHOUT
   * the `qrauth:ecdsa-canonical:v1:` domain prefix — required for JWS
   * verification by stock OIDC clients (`jose.jwtVerify`, etc.). Use only
   * for JWS / OIDC ID tokens. ADR-0003 Slice 3a.
   */
