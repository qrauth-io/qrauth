...
export class LoginEventService {
   ...
   * If the fingerprint matches a TRUSTED device, it's never suspicious.
   * If it matches a REVOKED device, it's always suspicious.
   */
  async isSuspiciousLogin(userId: string, meta: RequestMetadata): Promise<boolean> {
    // Check trusted device registry first
    if (meta.fingerprint) {
      const device = await this.prisma.trustedDevice.findUnique({
        where: { userId_fingerprint: { userId, fingerprint: meta.fingerprint } },
        select: { trustLevel: true, revokedAt: true },
      });

      if (device?.revokedAt) return true;
      if (device?.trustLevel === 'TRUSTED') return false;
    }
