...
export async function revokeOrphanedPending(
  ...
  const candidates = await deps.prisma.qRCode.findMany({
    where: {
      ...
      status: 'ACTIVE',
      createdAt: { lt: cutoff },
      // Issue #62 (62c). Seed-planted rows live outside the live
      // merkle pipeline — their batch flush is the seed run itself,
      // not a worker waiting on SLH-DSA. Skip them so a long-running
      // dev/test database doesn't get its seeds auto-revoked at the
      // 1-hour mark. Runtime rows (default 'runtime') and rows from
      // future bulk-import paths still flow through the revoke
      // branch unchanged.
      source: { not: 'seed' },
    },
    select: {
