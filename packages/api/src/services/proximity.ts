...
export class ProximityService {
  ...
  async createAttestation(input: CreateAttestationInput) {
    ...
    // 2. Get the org's active ES256 signing key. The algorithm filter is
    //    load-bearing: attestations are ES256 JWTs, and an org can also hold
    //    an RS256 OIDC key that must never be selected here.
    const signingKey = await this.prisma.signingKey.findFirst({
      where: { organizationId: qrCode.organizationId, algorithm: 'ES256', status: 'ACTIVE' },
  ...
  async verifyAttestation(
    ...
    if (!pubKey) {
      ...
      const signingKey = kid
        ...
        : await this.prisma.signingKey.findFirst({
            // No-kid fallback: attestations are ES256, so only ES256 keys
            // are verification candidates (the org may also hold RS256).
            where: { organizationId: orgId, algorithm: 'ES256', status: 'ACTIVE' },
            orderBy: { createdAt: 'desc' },
            select: { publicKey: true },
          });
