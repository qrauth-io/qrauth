...
export class HybridSigningService {
  ...
  /**
   * Pre-flight guard — assert the signing key's SLH-DSA material is
   * reachable before we charge the ECDSA leg and join a Merkle batch we
   * can't complete.
   *
   * Historically this always called `SigningService.loadSlhDsaKeyPair`,
   * which reads the encrypted `.slhdsa.enc` envelope off local disk. That
   * is correct for `SLH_DSA_SIGNER=local` (dev) but produces a hard
   * false-negative on `http` (production): the envelope lives ONLY on the
   * remote signer host per ADR-001 / Finding-016. A local lookup would
   * throw here on every QR issuance under the http backend — which is
   * exactly the regression that hit prod on 2026-04-20 against keyId
   * `005a92e9-...` after the org went https-backend-only.
   *
   * On `http` we skip the guard. The actual SLH-DSA sign still fires via
   * `BatchSigner.enqueue` → `HttpSlhDsaSigner`; if the remote signer is
   * missing the key, the sync path raises at enqueue time and the async
   * path surfaces the failure on the returned `merklePromise` (callers
   * already handle that, and the pending-reconciler reaps rows that
   * never complete). A remote "does this keyId exist?" ping is a
   * reasonable follow-up hardening but is not required to restore
   * issuance — it would only turn the late failure back into an early
   * one for the http backend.
   */
  private async assertSlhDsaMaterialAvailable(signingKeyId: string): Promise<void> {
    if (config.slhdsaSigner.backend === 'http') return;
    const slhPair = await this.signingService.loadSlhDsaKeyPair(signingKeyId);
    if (!slhPair) {
      throw new Error(
        `HybridSigningService: signing key "${signingKeyId}" has no SLH-DSA material. ` +
  ...
  async signSingleQR(input: HybridSignInput): Promise<HybridSignOutput> {
    await this.assertSlhDsaMaterialAvailable(input.signingKeyId);
