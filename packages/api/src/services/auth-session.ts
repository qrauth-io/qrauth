...
export class AuthSessionService {
  ...
  async approveSession(
    ...
    if (!isCliApp) {
      // Get the organization's active ES256 signing key (approval
      // signatures are ECDSA; the org's RS256 OIDC key must never be
      // selected here).
      const signingKey = await this.prisma.signingKey.findFirst({
        where: { organizationId: session.app.organizationId, algorithm: 'ES256', status: 'ACTIVE' },
      });
