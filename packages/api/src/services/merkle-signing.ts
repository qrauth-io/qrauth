...
export interface QRPayloadInput {
  /** Algorithm version string the leaf commits to. */
  algVersion: string;
  token: string;
  tenantId: string;
  /** Content type stored on the QR row (`"url"`, `"vcard"`, …). */
  contentType: string;
  /** Destination URL — used when contentType === "url". */
  destinationUrl: string;
  /** Hex content hash for non-URL QRs; ignored for URL QRs. */
...
export async function computeLeafHash(
  ...
  const canonical = canonicalizeMerkleLeaf({
...
export async function verifySignedBatch(
  ...
  // AUDIT-FINDING-010: signer prepends `qrauth:merkle-root:v1:` before
  // signing; verifier reconstructs the same prefixed input.
  ...
  const sigOk = await slhDsaVerify(tenantPublicKey, prefixedRoot, sigBuf);
...
export { CANONICAL_FIELD_SEPARATOR };
