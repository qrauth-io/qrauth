...
/**
 * ID-token lifetime. Exported because it is the binding constraint on the
 * JWKS publication window (lib/config.ts::assertJwksWindowAboveFloor): a
 * rotated key must stay published at least as long as a token it signed can
 * still be legitimately verified. Changing this value moves that floor
 * automatically — never let the two drift apart.
 */
export const ID_TOKEN_EXPIRES_IN_SECONDS = 3600;

const DEFAULT_EXPIRES_IN_SECONDS = ID_TOKEN_EXPIRES_IN_SECONDS;
