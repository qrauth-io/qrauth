...
/**
 * Connections handed to the named queues, retained so {@link closeQueues} can
 * release them. BullMQ's `queue.close()` does NOT disconnect a connection
 * passed in via the `connection` option — it treats it as caller-owned — so a
 * script that imports this module would otherwise keep the event loop alive
 * even after `closeQueues()` (T4-A: the OIDC bootstrap script lingered until a
 * manual `pkill` on prod 2026-05-31).
 */
const queueConnections: Redis[] = [];
 ...
 * Each call to this function returns a fresh connection, tracked for cleanup.
 */
...
export async function closeQueues(): Promise<void> {
  ...
  // `queue.close()` stops BullMQ but leaves the caller-owned ioredis
  // connections open, which keeps the event loop alive. Disconnect them so the
  // process can exit on its own (T4-A). `disconnect()` is immediate and, with
  // no reconnect, is safe here because the queues are already closed.
