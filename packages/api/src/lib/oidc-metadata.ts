...
/**
 * Platform-org slug + advertised signing algs live in oidc-signing-keys.ts
 * (single source for the signing/publishing key filters, the discovery
 * advertisement, and the key health check). Re-exported here for the many
 * existing importers.
 */
export { QRAUTH_PLATFORM_ORG_SLUG } from './oidc-signing-keys.js';
import { OIDC_ID_TOKEN_SIGNING_ALGS } from './oidc-signing-keys.js';
...
export function buildDiscoveryDocument(issuer: string) {
  return {
    ...
    // mandatory baseline); ES256 remains for opt-in clients. Derived from
    // the same constant the JWKS filter and key health check use, so the
    // advertised set cannot drift from the served set.
