...
const isDev = config.server.isDev || config.server.isTest;
 ...
 * - `sensitive`  — auth-critical endpoints (signup/login/reset), keyed by IP
 */
const TIERS: Record<string, { timeWindow: string; max: number; byIssuer: boolean }> = {
  public: {
    timeWindow: '1 minute',
    // Dev/test headroom mirrors the `sensitive` tier below: this tier is
    // also the plugin's GLOBAL default (any route without its own
    // config.rateLimit, e.g. the OIDC /authorize + /login surface), and the
    // E2E suite's now-mandatory OIDC token assertions push a single-IP run
    // past 100/min near the end. Production stays at 100.
    max: isDev ? 1000 : 100,
    byIssuer: false,
  },
  auth: {
...
export type RateLimitTier = 'public' | 'auth' | 'generate' | 'bulk' | 'sensitive';
...
export const rateLimitSensitive = rateLimitConfig('sensitive');
