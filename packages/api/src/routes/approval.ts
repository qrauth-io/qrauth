...
<body>
  <div class="card">
    <svg class="shield" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
      <path d="M48 24 H152 Q166 24 166 38 V108 Q166 156 100 182 Q34 156 34 108 V38 Q34 24 48 24 Z" fill="none" stroke="#081b4c" stroke-width="5"/>
      <g fill="#081b4c" opacity="0.15">
      ...
      <path d="M62 104 L88 134 L144 64" fill="none" stroke="#081b4c" stroke-width="15" stroke-linecap="round" stroke-linejoin="round"/>
    </svg>
    ...
    ` : `
      ...
      <div id="actions" style="display:none">
        <div id="user-info" style="padding:12px 16px;background:#f8f9fa;border-radius:8px;margin-bottom:16px;font-size:13px;color:#212b36;display:none;">
          ...
          <a href="#" id="switch-account" style="margin-left:8px;color:#0065DB;text-decoration:none;font-weight:600;">Not you?</a>
        </div>
        ...
        async function handleApprove() {
          ...
          try {
            ...
            if (res.ok) {
              ...
              if (REDIRECT_URL) {
                // OAuth-style callback: append qrauth_session_id +
                // qrauth_signature to the registered redirect URL so the
                // consumer can complete sign-in from URL params alone,
                // independent of any other tab. Critical on mobile where
                // the original tab (which was polling) may have been
                // suspended by the OS while the user was on this page —
                // when the suspension kills polling, the consumer can no
                // longer rely on its own qrauth:authenticated event.
                // Reads sessionId + signature from the /approve response.
                try {
                  var data = await res.json();
                  ...
                  // Body parse failure — fall back to bare redirect.
