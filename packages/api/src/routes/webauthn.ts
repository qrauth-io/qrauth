...
export default async function webauthnRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/register/verify', {
    ...
    const body = request.body as {
      credential: any;
      name?: string;
      /** AUDIT-FINDING-003: client-generated ML-DSA-44 public key, base64. */
    ...
    try {
      const { passkey, credentialDeviceType, bridgePolicy, bridgeEnabled } =
        await webauthnService.verifyRegistration(
          request.user!.id,
          body.credential,
          body.name,
          device?.id,
          body.bridgePublicKey,
          body.bridgeAlgorithm,
        );

      // AUDIT-FINDING-003: the server never sees, transits, or stores
      // the ML-DSA private key. The client generates the bridge keypair
      // locally, sends us the public half, and keeps the private half
      // in IndexedDB scoped to the rpId origin.
