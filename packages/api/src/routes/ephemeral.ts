...
/**
 * Resolve the org an ephemeral request is scoped to (ADR-0002 step 8).
 *
 * Two credential families, mutually exclusive by header:
 *   - X-API-Key / Bearer JWT → org-scoped credential. Org = request.user.orgId,
 *     no owning app. Write ops (create/revoke) reject VIEWER (mirrors qrcodes).
 *   - Basic / X-Client-Id (+ secret/PKCE) → app credentials. Org =
 *     app.organizationId, appId recorded as provenance.
 *
 * Returns null after sending the appropriate 401/403; the caller just returns.
 */
async function resolveEphemeralOrg(
  request: FastifyRequest,
  reply: FastifyReply,
  fastify: FastifyInstance,
  appService: AppService,
  opts: { allowPublicClient?: boolean; write?: boolean } = {},
): Promise<{ organizationId: string; appId: string | null } | null> {
  const hasApiKey = typeof request.headers['x-api-key'] === 'string' && request.headers['x-api-key']!.length > 0;
  const hasBearer = request.headers.authorization?.startsWith('Bearer ') ?? false;

  if (hasApiKey || hasBearer) {
    await fastify.authenticate(request, reply);
    if (reply.sent) return null; // 401 already sent by the auth middleware
    if (opts.write && request.user!.role === 'VIEWER') {
      reply.status(403).send({
        statusCode: 403,
        error: 'Forbidden',
        message: 'Your role does not permit creating or revoking ephemeral sessions.',
      });
      return null;
    }
    return { organizationId: request.user!.orgId, appId: null };
  }

  const app = await authenticateApp(request, reply, appService, opts.allowPublicClient ?? false);
  if (!app) return null;
  return { organizationId: app.organizationId, appId: app.id };
}
...
export default async function ephemeralRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/', {
    ...
    const ctx = await resolveEphemeralOrg(request, reply, fastify, appService, { allowPublicClient: true, write: true });
    ...
    // Ephemeral sessions have their own metric/cap
    // (PLAN_LIMITS.<tier>.ephemeralSessions). Quota is charged to the scoped
    // org regardless of which credential family created the session.
    ...
    try {
      const session = await sessionService.createSession({ organizationId: ctx.organizationId, appId: ctx.appId }, body);

      // Increment usage counter — error-tolerant in UsageService, awaited.
      await usageService.increment(ctx.organizationId, 'ephemeralSessions');
  ...
  fastify.get('/', {
    ...
    const ctx = await resolveEphemeralOrg(request, reply, fastify, appService);
    if (!ctx) return;
    ...
    try {
      const result = await sessionService.listSessions(ctx.organizationId, query);
      ...
      if (result.expiredSessionIds.length > 0) {
        ...
        for (const sessionId of result.expiredSessionIds) {
          webhookService.emit(ctx.organizationId, {
  ...
  fastify.get('/:id', { config: { rateLimit: rateLimitAuth } }, async (request, reply) => {
    const ctx = await resolveEphemeralOrg(request, reply, fastify, appService, { allowPublicClient: true });
    if (!ctx) return;
  ...
  fastify.delete('/:id', { config: { rateLimit: rateLimitAuth } }, async (request, reply) => {
    const ctx = await resolveEphemeralOrg(request, reply, fastify, appService, { write: true });
    if (!ctx) return;

    const { id } = request.params as { id: string };

    try {
      const result = await sessionService.revokeSession(id, ctx.organizationId);
