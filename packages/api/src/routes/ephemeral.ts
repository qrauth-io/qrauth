...
export default async function ephemeralRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/', {
    ...
    // Issue #65 (3). Ephemeral sessions get their own metric and their
    // own cap (PLAN_LIMITS.<tier>.ephemeralSessions). Until this PR they
    // shared the authSessions counter, which masked separate growth and
    // produced confusing dashboard numbers. The rename takes a one-time
    // counter reset (existing month's ephemeral counts under the
    // authSessions key are abandoned by Redis TTL) — acceptable for a
    // soft-meter migration.
    ...
    try {
      ...
      if (appRecord) {
        await usageService.increment(appRecord.organizationId, 'ephemeralSessions');
      }
