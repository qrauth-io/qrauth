...
export default async function authSessionRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/', {
    ...
    // Increment usage counter — UsageService.increment is already
    // error-tolerant (try/catch + warn log), so we await it directly.
    if (appRecord) {
      await usageService.increment(appRecord.organizationId, 'authSessions');
    }
    ...
    const qrImageDataUrl = await QRCode.toDataURL(qrUrl, { width: 440, margin: 2 });
    ...
    return reply.status(201).send({
      ...
      qrImageDataUrl, // rendered QR PNG (data: URL), generated server-side
  ...
  fastify.post('/:token/approve', {
    ...
    // First-party CLI flow (ADR-0002 §3, §5): scope the eventual API key to the
    // approver's org, resolved here on the trusted authenticated device. v0
    // auto-selects the sole membership; >1 or 0 is a clear error rather than a
    // guess. Third-party federation never enters this branch — targetOrg stays
    // undefined and that flow is unchanged.
    let targetOrganizationId: string | undefined;
    if (session.app.slug === QRAUTH_CLI_SLUG) {
      try {
        const membership = await sessionService.resolveSoleCliMembership(request.user!.id);
        targetOrganizationId = membership.organizationId;
      } catch (err) {
        if (err instanceof CliExchangeError) {
  ...
  // -----------------------------------------------------------------------
  // POST /:id/exchange — CLI credential exchange (ADR-0002 §3)
  // -----------------------------------------------------------------------
  // Public-client (PKCE) endpoint, qrauth-cli only. Exchanges an APPROVED
  // session for a freshly minted, org-scoped, single-use ApiKey. The raw key
  // is returned exactly once, only to the holder of the code_verifier.
  fastify.post('/:id/exchange', { config: { rateLimit: rateLimitAuth } }, async (request, reply) => {
    // Public client: X-Client-Id only, no secret.
    const app = await authenticateApp(request, reply, appService, true);
    if (!app) return; // 401 already sent

    const { id } = request.params as { id: string };
    const body = request.body as { code_verifier?: string; hostLabel?: string } | undefined;
    const query = request.query as { code_verifier?: string };
    ...
    // The exchange mints a high-value org-scoped credential, so it is locked to
    // the first-party qrauth-cli app — no other (third-party) app can call it.
    const cliApp = await appService.findByClientId(QRAUTH_CLI_CLIENT_ID);
    if (!cliApp || app.id !== cliApp.id) {
      return reply.status(403).send({
        statusCode: 403,
        error: 'Forbidden',
        message: 'The credential exchange is only available to the qrauth-cli app.',
      });
    }

    // hostLabel is operator-supplied display text for the key label; keep only
    // filename-safe chars and cap length so it can't smuggle anything in.
    const hostLabel = body?.hostLabel?.replace(/[^\w.-]/g, '').slice(0, 64) || undefined;

    try {
      const result = await sessionService.exchangeForApiKey({
        sessionId: id,
        cliAppId: cliApp.id,
        codeVerifier,
        hostLabel,
      ...
      return reply.send({
        apiKey: result.apiKey, // SHOWN ONCE
  ...
  fastify.post('/verify-result', { config: { rateLimit: rateLimitPublic } }, async (request, reply) => {
    ...
    const valid = cryptoValid && isApproved;

    // AUDIT-FINDING (pentest AUTH-001): user PII and the signature envelope
    // must NOT be returned unless the caller proved possession via a valid
    // ECDSA signature. Previously these were emitted regardless of `valid`,
    // so anyone who knew a sessionId could read name/email and obtain a
    ...
    return reply.send({
      ...
      session: {
        ...
        scopes: session.scopes,
        user: valid && session.user ? {
          id: session.user.id,
          ...(session.scopes.includes('identity') ? { name: session.user.name } : {}),
          ...(session.scopes.includes('email') ? { email: session.user.email } : {}),
        } : null,
        ...(valid ? { signature: session.signature } : {}),
