...
export default async function authRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/switch-org', {
    ...
    // F-012 (audit 2026-05-17): record the org pivot. If a user with
    // access to multiple orgs is later found to be compromised, this
    // trail lets us reconstruct which tenants they touched.
    await auditService.log({
      ...
      metadata: {
        fromOrgId: request.user!.orgId,
        toOrgId: body.organizationId,
      },
    });
  ...
  // roundtrip. Mounted only when NODE_ENV !== 'production' AND
  // ENABLE_TEST_HOOKS=1 (config.server.testHooksEnabled) — the explicit
  // opt-in exists because NODE_ENV is optional-with-default, so an unset
  // NODE_ENV alone must never expose these routes. Kept intentionally
  // minimal — takes {email}, flips the flag, no auth.
  if (config.server.testHooksEnabled) {
    ...
    // -----------------------------------------------------------------------
    // POST /_test/ensure-platform-signing-keys (DEV/TEST ONLY)
    // -----------------------------------------------------------------------
    //
    // E2E fixture for the OIDC specs: guarantees the platform org holds one
    // ACTIVE ES256 and one ACTIVE RS256 signing key, so the RS256 ID-token
    // assertions RUN instead of silently skipping — a skip on exactly the
    // missing-RS256-key condition is how the rotation-loop incident
    // (docs/ops/signing-key-rotation-loop.md) stayed invisible to a green
    // suite. Keys are minted INSIDE the API process via the audited
    // SigningService paths, so their at-rest envelopes decrypt with this
    // process's master key and the OP can actually sign with them (an
    // external bootstrap script's envelopes cannot be decrypted here unless
    // SIGNER_MASTER_KEY is shared). Idempotent; mounted only when
    // NODE_ENV !== 'production'.
    fastify.post('/_test/ensure-platform-signing-keys', {
      config: { rateLimit: rateLimitPublic },
      ...
      if (!org) {
        return reply.status(500).send({ error: 'platform org missing — run prisma migrate deploy' });
      }

      const ensured: Record<string, string> = {};
      for (const algorithm of ['ES256', 'RS256'] as const) {
        const existing = await fastify.prisma.signingKey.findFirst({
          where: { organizationId: org.id, algorithm, status: 'ACTIVE' },
          select: { keyId: true },
        });
        if (existing) {
          ensured[algorithm] = existing.keyId;
          continue;
        }
        const created =
          algorithm === 'ES256'
            ? await fastify.signingService.createKeyPair(org.id)
            : await fastify.signingService.createRsaKeyPair(org.id);
        ensured[algorithm] = created.keyId;
      }
      return reply.send({ ensured });
    });
  }
  ...
  fastify.post('/onboarding/complete', {
    ...
    // Idempotent: applies org name/slug/trustLevel + signing key only on the
    // FIRST completion. An already-onboarded user re-running /onboarding must
    // not overwrite the org (the slug feeds the public verification page).
    const { applied } = await completeOnboarding(
      fastify.prisma,
      { userId, orgId, organizationName, useCase },
      async (id) => {
        const existingKey = await fastify.prisma.signingKey.findFirst({
          // createKeyPair mints ES256, so the idempotency check must be
          // algorithm-scoped too (an RS256-only org still needs one).
          where: { organizationId: id, algorithm: 'ES256', status: 'ACTIVE' },
        });
        if (!existingKey) {
          await fastify.signingService.createKeyPair(id);
        }
      },
    );

    return reply.send({ success: true, alreadyOnboarded: !applied });
  });
