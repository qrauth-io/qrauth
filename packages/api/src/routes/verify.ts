  ...
  fastify.get('/:token', {
    ...
    if (!acceptsJson) {
      ...
      if (renderer) {
        ...
        // F-007 (audit 2026-05-17): the shell now returns { html, nonce }
        // so we can set a per-response CSP that authorises the inline
        // <style> + <script> blocks via the nonce instead of relying on
        // 'unsafe-inline'. The late onSend hook in server.ts picks up
        // request.cspOverride and rewrites the header for this response.
        const { html, nonce } = renderShell(renderCtx, contentBody, proofHmac ?? undefined);
