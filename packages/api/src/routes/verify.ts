...
import {
  ...
  renderGeoBlockPage,
  type GeoBlockReason,
} from '../renderers/geo-gate.js';
import type { RenderContext } from '../renderers/index.js';
import { UsageService } from '../services/usage.js';
import { attachQuotaHeaders, shouldMeterVerification } from './verify-quota-headers.js';
...
export default async function verifyRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  // Verify only uses getQuickTrustScore, which never reaches the alerting
  // paths — so no AlertService is wired (the ctor param is `AlertService | null`).
  const fraudService = new FraudDetectionService(fastify.prisma, geoService, null);
  ...
  fastify.get('/:token', {
      ...
      include: {
        organization: {
          select: {
            ...
            kycStatus: true,
            domainVerified: true,
            // Issue #65: needed to compute the X-Quota-* response
            // headers (limit lookup against PLAN_LIMITS).
            plan: true,
          },
        },
        signingKey: {
    ...
    if (!qrCode) {
      return reply.status(404).send(notFoundError('QR code', token));
    }
    ...
    // plus the batch root. Canonical construction is shared with the MAC
    // re-keying path (lib/qr-canonical.ts) — issuer, verifier, and re-keyer
    // MUST produce identical bytes.
    const coreCanonical = await buildQrCanonicalCore(qrCode);
    ...
    if (qrCode.transparencyLogEntry) {
      try {
        ...
        fastify.log.warn(
          { err, qrCodeId: qrCode.id, token: qrCode.token },
          'Transparency-log inclusion proof check failed; marking unverified',
        );
        transparencyLogVerified = false;
      }
    }
    ...
    // ------------------------------------------------------------------
    // 6b. Geo-fence enforcement gate
    // ------------------------------------------------------------------
    // When the QR has `requireProximity` set (the default for new geo-bound
    // rows) and is bound to coordinates, we fail closed in two cases:
    //   - The scanner did not supply coords at all → GEO_PROXIMITY_REQUIRED.
    //     HTML clients are diverted to a GPS-request interstitial; JSON
    //     clients receive `verified: false` and a machine-readable reason.
    //   - The scanner supplied coords that landed outside the radius →
    //     GEO_OUT_OF_FENCE. Both surfaces return `verified: false`.
    // When `requireProximity` is false, the binding stays advisory: distance
    // is still surfaced in `location_match`, but `verified` is unaffected.
    const isGeoBound = qrCode.latitude !== null && qrCode.longitude !== null;
    const geoEnforced = isGeoBound && qrCode.requireProximity === true;
    const clientSuppliedCoords =
      query.clientLat !== undefined && query.clientLng !== undefined;

    let geoBlocked: GeoBlockReason | null = null;
    if (geoEnforced) {
      if (!clientSuppliedCoords) {
        geoBlocked = 'GEO_PROXIMITY_REQUIRED';
      } else if (!locationMatch.matched) {
        geoBlocked = 'GEO_OUT_OF_FENCE';
      }
    }
      ...
      fastify.log.warn(
        {
          ...
          kind: 'GEO_BLOCK',
          reason: geoBlocked,
          distanceM: locationMatch.distanceM,
          radiusM: qrCode.radiusM,
        },
        'verify: geo-fence block',
      );
    }
    ...
    const cryptoVerified = signatureValid && finalTrustScore > 20;
    const verifiedFinal = cryptoVerified && !geoBlocked;
    const geoReason: string | undefined = geoBlocked ?? undefined;
    const staticParts = {
      verified: verifiedFinal,
      ...(geoBlocked
        ? { reason: geoReason as VerificationFailureReason }
        : signatureValid && finalTrustScore <= 20
          ? { reason: 'QR code has a critically low trust score due to detected fraud.' }
          : {}),
    ...
    //
    // Skip the cache entirely for geo-enforced rows: `verified` now depends on
    // per-request coordinates, so caching `staticParts` would risk serving an
    // in-fence success back to an out-of-fence scanner (and vice versa).
    if (!geoEnforced) {
    ...
    scanQueue
      .add('record-scan', {
        ...
        metadata: {
          signatureValid,
          token,
          // Geo enforcement signals — the fraud dashboard and AI agent
          // use these to distinguish a denied-GPS user from a real
          // out-of-area attempt, and the analytics pipeline filters them
          // out of "successful verification" rollups.
          geoEnforced,
          geoBlocked,
        },
      })
    ...
    // ------------------------------------------------------------------
    // Soft verifications meter (Issue #65, 5A/5B/5C).
    //
    // Counts SUCCESSFUL verifications only — the increment is gated on
    // `signatureValid`. Failed-signature attempts are not chargeable
    // and would otherwise let an attacker spam the meter for free.
    //
    // No 429 is returned here under any circumstances. The meter is a
    // billing/observability signal, not a hard cap. Anti-abuse hard
    // capping (e.g. 200 % of plan limit) is filed separately as #84.
    //
    // UsageService.increment owns its own try/catch + structured warn
    // log, so a Redis outage no longer fails the verify request — it
    // returns 0 from the counter and the verification response stays
    // unchanged.
    // ------------------------------------------------------------------
    const usageService = new UsageService(fastify.prisma, fastify.log);
    // Geo-blocked attempts are not chargeable: the QR's cryptographic
    // verification may have passed, but the scan never reached the user-
    // facing destination. Metering them would punish issuers for legitimate
    // out-of-fence scans they had no control over.
    if (shouldMeterVerification(signatureValid) && !geoBlocked) {
      await usageService.increment(qrCode.organizationId, 'verifications');
    }
    await attachQuotaHeaders(reply, usageService, qrCode.organizationId, qrCode.organization.plan);
    ...
    if (!acceptsJson) {
      // Geo-fence enforcement diverts BEFORE the Trust Reveal / content shell.
      // - GEO_PROXIMITY_REQUIRED → GPS interstitial that re-loads the page
      //   with `?clientLat=…&clientLng=…` so the second hit can run the
      //   proximity check.
      // - GEO_OUT_OF_FENCE → terminal deny page; no content, no CTA.
      if (geoBlocked) {
        const { html: geoHtml, nonce: geoNonce } =
          geoBlocked === 'GEO_PROXIMITY_REQUIRED'
            ? renderGeoGatePage({
                token,
                orgName: qrCode.organization.name,
                scannedAt,
              })
            : renderGeoBlockPage({
                token,
                ...
                reason: geoBlocked,
                scannedAt,
                distanceM: locationMatch.distanceM ?? undefined,
                radiusM: qrCode.radiusM,
              });
        (request as any).cspOverride = [
          "default-src 'self'",
          `script-src 'self' 'nonce-${geoNonce}'`,
          `style-src 'self' 'nonce-${geoNonce}'`,
        ...
        return reply.type('text/html').send(geoHtml);
      }
      ...
      if (renderer) {
        // F-007 (audit 2026-05-17): generate ONE per-response CSP nonce BEFORE
        // rendering content, so the SAME value flows into the content renderer
        // (via RenderContext.nonce), the shell, and the CSP header below — no
        // directive needs 'unsafe-inline'. 128 bits, base64 → 24 chars.
        ...
        // F-007 (audit 2026-05-17): the shell and every content renderer nonce
        // their inline <style>/<script> blocks with renderCtx.nonce, so the
        // per-response CSP below authorises them via the nonce instead of
        // 'unsafe-inline'. The late onSend hook in server.ts picks up
        // request.cspOverride and rewrites the header for this response.
        const { html } = renderShell(renderCtx, contentBody, proofHmac ?? undefined);
        // F-007 (audit 2026-05-17): CSP3 split of style-src vs
        // style-src-attr. The nonce locks down inline <style> blocks
        // (the load-bearing XSS path — `background-image: url(...)` can
        // exfil data). style-src-attr 'unsafe-inline' allows style="…"
        // attributes that the shell + content renderers use for dynamic
        // colors. Inline style ATTRIBUTES can't reference external URLs
        // or execute JS — risk is narrow.
        (request as any).cspOverride = [
          "default-src 'self'",
      ...
      return reply.type('text/html').send(renderVerificationPage(response, token, qrCode, fastify.log, ephemeralProof));
    }
  ...
  fastify.post('/:token/webauthn/options', {
    ...
    if (!qrCode || qrCode.status !== 'ACTIVE') {
      return reply.status(404).send(notFoundError('QR code'));
    }
  ...
  fastify.post('/:token/feedback', {
    ...
    if (!qrCode || qrCode.contentType !== 'feedback') {
      return reply.status(404).send(notFoundError('Feedback QR code'));
    }
...
function renderVerificationPage(
  ...
  try {
    ...
    log.warn(
      { err, token, destinationUrl: qrCode.destinationUrl },
      'Failed to parse QR destinationUrl; falling back to the raw value for hostname display',
    );
