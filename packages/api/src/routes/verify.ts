...
export default async function verifyRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get('/:token', {
    ...
    // Symmetric MAC fast path (ALGORITHM.md §7). If the row carries a MAC,
    // recompute it from the org's HMAC key ring. A match confirms the QR is
    // server-authentic via pure symmetric crypto and we short-circuit the
    // asymmetric legs. A miss is NOT proof of forgery — keys may have
    // rotated — so we fall through to ECDSA + Merkle/SLH-DSA which remain
    // authoritative.
    let signatureValid = false;
    let pqcReason: string | null = null;
    let fastPathHit = false;
    // Merkle leg state surfaced in the response under `security`.
    // Defaults: for legacy ECDSA-only rows, `merkleProofValid` is true by
    // convention (there is no merkle leg to fail) and `merkleBatchId` is
    // null. For hybrid rows, both are populated below.
    let merkleProofValid = true;
    let merkleBatchId: string | null = qrCode.merkleBatchId ?? null;

    if (qrCode.macTokenMac) {
      const macCanonical = hashPayload(
        qrCode.token,
      ...
      fastPathHit = await macService.verifyCanonical({
      ...
      if (fastPathHit) signatureValid = true;
    }
    ...
    if (!signatureValid) {
      ...
      // Hybrid signing: rows with `algVersion = hybrid-ecdsa-slhdsa-v1` carry
      // a Merkle inclusion proof + SLH-DSA-signed batch root. BOTH the legacy
      // ECDSA leg and the new SLH-DSA/Merkle leg must verify; if either fails
      // we treat the QR as cryptographically invalid (ALGORITHM.md §6.4).
      let pqcValid = true;
      if (
        qrCode.algVersion === 'hybrid-ecdsa-slhdsa-v1' &&
        qrCode.merkleBatchId &&
        qrCode.merkleLeafHash &&
        qrCode.merklePath
      ) {
        merkleBatchId = qrCode.merkleBatchId;
        const result = await hybridSigningService.verifyHybridLeg({
        ...
        if (!result.ok) {
          ...
          fastify.log.warn(
            { token: qrCode.token, reason: result.reason },
            'Hybrid PQC verification failed',
          );
        }
      }

      signatureValid = ecdsaValid && pqcValid;
      if (!signatureValid && pqcReason) {
        // Surface the PQC failure reason on the request log for forensics.
        fastify.log.warn({ token: qrCode.token, pqcReason }, 'Signature verification failed');
      }
    }

    if (fastPathHit) {
      fastify.log.debug({ token: qrCode.token }, 'verify: MAC fast path hit');
    }
    ...
    const staticParts = {
      ...
      security: {
        ...
        // leg in addition to (or instead of) the classical ECDSA leg.
        // Legacy ECDSA-only rows are explicitly false so operator
        // dashboards can surface the gap.
        pqcProtected:
          qrCode.algVersion !== null &&
          qrCode.algVersion !== ALG_VERSION_POLICY.legacyEcdsa,
        merkleProofValid,
        merkleBatchId,
      },
    ...
    if (!acceptsJson) {
      ...
      if (renderer) {
        ...
        return reply.type('text/html').send(renderShell(renderCtx, contentBody, proofHmac));
      }
