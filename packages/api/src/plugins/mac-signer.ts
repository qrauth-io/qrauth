...
export function buildCircuitOpenLogRecord(args: {
  ...
  return {
    ...
    message: 'MAC signer circuit breaker opened — signer RPCs short-circuited',
  };
}
  ...
  if (backend === 'local') {
    ...
    if (backend === 'signer') {
      fastify.log.info(
        ...
        'MAC_BACKEND=signer active — Phase 3 posture. ' +
          'Signer is the SOLE verifier for animated-QR frames. ' +
          'No local HKDF fallback: signer unavailability hard-fails verification.',
      );
    } else {
      fastify.log.info(
        { backend, url: config.macSigner.url },
    ...
    circuit = createCircuitBreaker({
      ...
      onOpen: () => {
        // Severity: `error`, not `fatal`. A tripped breaker is a degraded
        // state, not a process-death signal — the rest of the API stays
        // up. In signer mode (Phase 3) it means animated-QR verification
        // hard-fails until the breaker closes; in dual mode local stays
        // authoritative. The structured `event` field is the discriminator
