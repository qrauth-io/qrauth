...
End by writing a daily security report with your findings.`,
    ...
    while (iterations < maxIterations) {
      ...
      if (response.stop_reason === 'tool_use') {
        ...
        for (const block of assistantContent) {
          if (block.type === 'tool_use') {
            ...
            // F-003: pass agentRun.id so create_fraud_rule can enforce
            // the per-run cap. F-010: organizationId left null because
            // the daily analysis is platform-wide by design (the call
            // is logged with an [agent.global_query] prefix).
      ...
      // Any other stop reason (refusal, max_tokens) — looping again would
      // resend a trailing assistant turn, which the API rejects as a prefill.
      console.error(`[agent] Stopping on unexpected stop_reason: ${response.stop_reason}`);
      break;
    }
