...
import { FraudRuleInputSchema } from '@qrauth/shared';
 ...
 * ========================================================================
 *
 * The tools in this file may execute GLOBAL Prisma queries when
 * `organizationId` is null. That is intentional: the daily-analysis agent
 * runs as a server-side background job and looks at platform-wide data to
 * detect cross-tenant attack patterns.
 *
 * NEVER call `executeTool(..., agentRunId, null)` from a request-scoped
 * HTTP handler. Any future surface that exposes these tools to a user
 * must pass a non-null `organizationId` derived from the authenticated
 * Membership; otherwise it becomes a cross-tenant data exfiltration path
 * (F-010, docs/security/audit-2026-05-17-api-and-server.md).
 *
 * Global-scope calls are logged to stdout with an `[agent.global_query]`
 * prefix so operators can audit them.
 * ========================================================================
 */

// F-003 (audit 2026-05-17): hard cap on rules an agent run may create.
// Prevents a single bad run from spamming dozens of overly-permissive
// rules. The agent sees the cap error as a tool result and can adjust.
const MAX_RULES_PER_RUN = 5;
const runRuleCounters = new Map<string, number>();
 ...
 *
 * @param name           Tool identifier, must match one of TOOL_DEFINITIONS
 * @param input          Tool input (validated per-tool; outer shape is opaque)
 * @param agentRunId     The AgentRun.id this call is part of. Used for
 *                       per-run rule-creation rate limiting (F-003) and
 *                       structured audit logging.
 * @param organizationId Tenant scope. `null` runs platform-wide (only safe
 *                       for the background daily-analysis job — see file
 *                       header). Non-null restricts queries to that org.
 */
export async function executeTool(
  ...
  // F-010: emit a structured audit line every time a tool runs in global
  // scope. Operators grep this prefix to confirm only the daily job
  // appears. A request-scoped handler accidentally calling executeTool
  // with null will surface here immediately.
  ...
  // Helper: build a Prisma where-clause add-on that scopes by org via the
  // qrCode relation when present. Used by tools that query Scan /
  // FraudIncident records.
  const orgScopeViaQrCode = organizationId ? { qrCode: { organizationId } } : {};
    ...
    switch (name) {
      case 'query_scan_patterns': {
        ...
        if (input.groupBy === 'hour') {
          const scans = await prisma.scan.findMany({
            where: { createdAt: { gte: since }, ...orgScopeViaQrCode },
        ...
        const [total, avgTrust, proxyCount] = await Promise.all([
          prisma.scan.count({ where: { createdAt: { gte: since }, ...orgScopeViaQrCode } }),
          prisma.scan.aggregate({ where: { createdAt: { gte: since }, ...orgScopeViaQrCode }, _avg: { trustScore: true } }),
          prisma.scan.count({ where: { createdAt: { gte: since }, proxyDetected: true, ...orgScopeViaQrCode } }),
        ]);
      ...
      case 'query_fraud_incidents': {
        ...
        if (input.groupBy === 'type') {
          ...
          const resolved = await prisma.fraudIncident.count({ where: { createdAt: { gte: since }, resolved: true, ...orgScopeViaQrCode } });
          const total = await prisma.fraudIncident.count({ where: { createdAt: { gte: since }, ...orgScopeViaQrCode } });
      ...
      case 'query_login_events': {
        const since = getTimeRange(input.timeRange as string);
        // LoginEvent has a direct organizationId column when the login is
        // tied to an org context; scope via that when an org is supplied.
        const loginOrgScope = organizationId ? { organizationId } : {};
        if (input.groupBy === 'country') {
          const groups = await prisma.loginEvent.groupBy({
            by: ['ipCountry'],
            where: { createdAt: { gte: since }, ipCountry: { not: null }, ...loginOrgScope },
            _count: true,
        ...
        if (input.groupBy === 'success') {
          const [success, failed] = await Promise.all([
            prisma.loginEvent.count({ where: { createdAt: { gte: since }, success: true, ...loginOrgScope } }),
            prisma.loginEvent.count({ where: { createdAt: { gte: since }, success: false, ...loginOrgScope } }),
          ]);
          return JSON.stringify({ successful: success, failed, failRate: success + failed > 0 ? Math.round((failed / (success + failed)) * 100) : 0 });
        }
        const total = await prisma.loginEvent.count({ where: { createdAt: { gte: since }, ...loginOrgScope } });
        const failed = await prisma.loginEvent.count({ where: { createdAt: { gte: since }, success: false, ...loginOrgScope } });
        return JSON.stringify({ total, failed });
      }

      case 'query_org_activity': {
        const since = getTimeRange(input.timeRange as string);
        // When scoped to a single org, return only that org.
      ...
      case 'query_feature_snapshots': {
        // When an org is supplied, only return snapshots scoped to that
        // entity (entityType === 'organization' && entityId === orgId).
        // Global snapshots stay accessible to the unscoped daily job.
        const snapshotOrgScope = organizationId
      ...
      case 'create_fraud_rule': {
        // F-003: enforce the per-run cap BEFORE validating, so a runaway
        // agent learns about the cap immediately rather than burning
        // tokens on rule drafting.
        const count = runRuleCounters.get(agentRunId) ?? 0;
        if (count >= MAX_RULES_PER_RUN) {
          return JSON.stringify({
            error:
              `Per-run rule creation limit reached (${MAX_RULES_PER_RUN}). ` +
              `No more rules can be created in this agent run. ` +
              `Consider revising existing rules or escalating to an operator.`,
          });
        }

        // F-003: Zod validation before persisting. Malformed input throws,
        // caught by the outer try/catch, and the error surfaces to the
        // agent as a tool result so it can correct itself.
        const parsed = FraudRuleInputSchema.parse(input);

        const rule = await prisma.fraudRule.create({
          data: {
            name: parsed.name,
            description: parsed.description ?? '',
            conditions: parsed.conditions as unknown as object,
            action: parsed.action as unknown as object,
            priority: parsed.priority,
            source: 'agent',
          },
        });

        runRuleCounters.set(agentRunId, count + 1);

        // F-003: structured audit log. AuditLog table requires non-null
        // organizationId + userId (FK constraints) and agent-created rules
        // are global, so we log to stdout with an [agent.fraud_rule.create]
        // prefix. The prisma.fraudRule row + source='agent' is the
        // durable record; this line surfaces the agentRunId linkage.
        console.info(
          '[agent.fraud_rule.create] ' +
            JSON.stringify({
              agentRunId,
              ruleId: rule.id,
              name: parsed.name,
              ...
              priority: parsed.priority,
              runRuleCount: count + 1,
              maxPerRun: MAX_RULES_PER_RUN,
              at: new Date().toISOString(),
            }),
        );
        ...
        return JSON.stringify({
          ...
          ruleId: rule.id,
          name: rule.name,
          remainingInRun: MAX_RULES_PER_RUN - (count + 1),
        });
      }
      ...
      case 'write_report': {
        ...
        // Send email if configured. ADMIN_EMAILS is a comma-separated list
        // (shared with routes/admin.ts and routes/analytics.ts); the daily
        // report goes to every address in it.
        ...
        return JSON.stringify({ stored: true, reportId: report.id, emailed: adminEmails.length > 0 });
      }
