...
function renderEventContent(ctx: RenderContext): string {
  ...
  return `
    <style nonce="${ctx.nonce}">
      ...
      .event-title { font-size: 22px; font-weight: 800; color: #0A0A0A; margin-bottom: 4px; }
      .event-organizer { font-size: 13px; color: #525252; margin-bottom: 16px; }
      .event-desc { font-size: 14px; color: #525252; line-height: 1.6; margin-bottom: 16px; }
      .event-field { display: flex; align-items: flex-start; gap: 12px; padding: 10px 0; border-bottom: 1px solid #FAFAF9; }
      ...
      .event-label { font-size: 11px; color: #737373; text-transform: uppercase; }
      .event-value { font-size: 14px; color: #262626; }
      .event-cta { display: block; width: 100%; padding: 14px; margin-top: 20px; background: #0A0A0A; color: #ffffff; border: none; border-radius: 10px; font-size: 16px; font-weight: 600; text-align: center; text-decoration: none; }
      a.event-cta, a.event-cta:link, a.event-cta:visited { color: #ffffff; }
      .event-cta:hover { background: #262626; }
    </style>

    ${c.imageUrl ? `<img class="event-img" src="${esc(c.imageUrl)}" alt="" data-hide-on-error />` : ''}
    ...
    <div class="event-field">
      <div class="event-icon" style="background:#EEF2FF;">&#128197;</div>
      <div>
    ...
    <div class="event-field">
      <div class="event-icon" style="background:#F5F3FF;">&#128205;</div>
      <div>
        <div class="event-label">Where</div>
        ${c.location ? `<div class="event-value">${esc(c.location)}</div>` : ''}
        ${c.address ? `<div class="event-value" style="font-size:13px;color:#525252;">${esc(c.address)}</div>` : ''}
      </div>
    </div>
    ` : ''}
    ...
    <div class="event-field">
      ...
      <div>
        ...
        <div class="event-value"><a href="mailto:${esc(c.contactEmail)}" style="color:#4F46E5;text-decoration:none;">${esc(c.contactEmail)}</a></div>
      </div>
    </div>
    ` : ''}
    ...
    ${c.website ? `<a href="${esc(c.website)}" target="_blank" rel="noopener" style="display:block;text-align:center;margin-top:12px;font-size:13px;color:#4F46E5;">Event Website &#8594;</a>` : ''}
  `;
}
