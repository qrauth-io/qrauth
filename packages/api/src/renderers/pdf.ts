...
function renderPdfContent(ctx: RenderContext): string {
  ...
  return `
    <style nonce="${ctx.nonce}">
      .pdf-title { font-size: 20px; font-weight: 800; color: #0A0A0A; margin-bottom: 4px; text-align: center; }
      .pdf-desc { font-size: 14px; color: #525252; text-align: center; margin-bottom: 20px; line-height: 1.6; }
      .pdf-viewer { width: 100%; height: 400px; border: 1px solid #f0f0f0; border-radius: 8px; }
      .pdf-download { display: block; width: 100%; padding: 14px; margin-top: 16px; background: #0A0A0A; color: #ffffff; border: none; border-radius: 10px; font-size: 16px; font-weight: 600; text-align: center; text-decoration: none; }
      .pdf-download:hover { background: #262626; }
    </style>
    ...
    ` : `
      <div style="padding:40px;text-align:center;background:#FAFAF9;border-radius:8px;color:#737373;">
