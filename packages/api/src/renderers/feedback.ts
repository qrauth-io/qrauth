...
function renderFeedbackContent(ctx: RenderContext): string {
  ...
  return `
    <style nonce="${ctx.nonce}">
      .fb-title { font-size: 22px; font-weight: 800; color: #0A0A0A; text-align: center; margin-bottom: 8px; }
      .fb-desc { font-size: 14px; color: #525252; text-align: center; margin-bottom: 24px; line-height: 1.6; }
      .fb-stars { display: flex; justify-content: center; gap: 8px; margin-bottom: 20px; }
      .fb-star { font-size: 36px; cursor: pointer; color: #E5E5E5; transition: color 0.2s; user-select: none; }
      .fb-star.active, .fb-star:hover { color: #B45309; }
      .fb-input { width: 100%; padding: 10px 14px; border: 1px solid #E5E5E5; border-radius: 8px; font-size: 14px; font-family: inherit; box-sizing: border-box; margin-bottom: 10px; }
      .fb-input:focus { outline: none; border-color: #047857; }
      .fb-textarea { width: 100%; padding: 12px; border: 1px solid #E5E5E5; border-radius: 8px; font-size: 14px; font-family: inherit; resize: vertical; min-height: 80px; box-sizing: border-box; }
      .fb-textarea:focus { outline: none; border-color: #047857; }
      .fb-submit { display: block; width: 100%; padding: 14px; margin-top: 16px; background: #047857; color: white; border: none; border-radius: 10px; font-size: 16px; font-weight: 600; cursor: pointer; }
      .fb-submit:hover { background: #047857; }
      .fb-submit:disabled { background: #737373; cursor: not-allowed; }
      .fb-thanks { text-align: center; padding: 24px; display: none; }
      .fb-thanks h3 { color: #047857; margin-bottom: 8px; }
    </style>
    ...
    <div id="fb-form">
      ...
      <div class="fb-stars" id="fb-stars">
        ${[1,2,3,4,5].map(n => `<span class="fb-star" data-rating="${n}">&#9733;</span>`).join('')}
      </div>
    ...
    <div class="fb-thanks" id="fb-thanks">
      ...
      <p style="color:#525252;font-size:14px;">Your response has been recorded.</p>
    </div>
    ...
    <script nonce="${ctx.nonce}">
      ...
      // CSP F-007: inline click-handler attributes are blocked by script-src
      // (no 'unsafe-inline'); bind interactivity here in the nonced script
      // instead. This block runs after the form markup above is parsed, so the
      // elements already exist.
      document.querySelectorAll('.fb-star').forEach(function(star) {
        star.addEventListener('click', function() {
          setRating(parseInt(star.getAttribute('data-rating'), 10));
        });
      });
