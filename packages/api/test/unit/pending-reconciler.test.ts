  ...
  qRCode = {
    findMany: async (args: FakePrismaFilter & { take?: number; select?: unknown }) => {
      const all = [...this.rows.values()].filter((r) => {
        ...
        if (args.where.source && r.source === args.where.source.not) return false;
        return true;
      });
...
function makePending(id: string, ageMs: number, source: string | null = 'runtime'): FakeQRCode {
  return {
    id,
    token: `tok_${id}`,
...
describe('revokeOrphanedPending (AUDIT-2 N-1)', () => {
  ...
  // Issue #62 (62c). Seed-planted rows live outside the live merkle
  // pipeline — their lifecycle is owned by the seed run, not a worker
  // waiting on SLH-DSA. The reconciler must skip them so a long-running
  // dev/test database doesn't get its seeds auto-revoked at the 1-hour
  // mark.
  it('skips a seed-sourced row even when it is well past the threshold', async () => {
    prisma.rows.set('seeded-old', makePending('seeded-old', 5 * 60 * 60 * 1000, 'seed'));
    ...
    const row = prisma.rows.get('seeded-old')!;
    expect(row.status).toBe('ACTIVE');
    expect(row.revocationReason).toBeNull();
    expect(webhookService.calls).toHaveLength(0);
    expect(logger.info).not.toHaveBeenCalled();
  ...
  it('still revokes runtime rows when seed and runtime rows are mixed in the same batch', async () => {
    prisma.rows.set('seeded', makePending('seeded', 99 * 60 * 1000, 'seed'));
    prisma.rows.set('runtime', makePending('runtime', 99 * 60 * 1000, 'runtime'));

    const result = await revokeOrphanedPending(deps(), { nowMs: NOW });

    expect(result.scanned).toBe(1);
    expect(result.revoked).toBe(1);
    expect(prisma.rows.get('seeded')!.status).toBe('ACTIVE');
    expect(prisma.rows.get('runtime')!.status).toBe('REVOKED');
    expect(webhookService.calls.map((c) => c.data.qrCodeId)).toEqual(['runtime']);
  });
