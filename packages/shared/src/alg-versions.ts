...
export const ALG_VERSION_POLICY = {
   ...
   * ECDSA + MAC alone — a temporary defense-in-depth gap. The reconciler
   * upgrades the row to `hybrid` within minutes.
   */
...
/**
 * Versions that pass verification but emit a warning. Currently empty —
 * `legacyEcdsa` was dropped entirely by the canonical-form unification
 * (AUDIT-FINDING-011/019/020/021), so there is no deprecated alg version
 * in the active policy. Future sunsets land here before being moved to
 ...
 * `ALG_VERSION_REJECTED`. Any string not in any other set also fails
 * with `unknown`, so the net behaviour is identical either way.
 */
