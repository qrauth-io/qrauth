...
import { httpUrl, domainName, safeImageUrl } from "./http-url.js";
...
/**
 * Fields an org OWNER/ADMIN may change about their OWN org. Excludes
 * `plan`, `trustLevel`, and any billing/KYC-controlled field — those are
 * written ONLY by the Stripe webhook (billing.ts) and admin routes (admin.ts).
 * (pentest IDOR-001: prevents self-upgrade to ENTERPRISE / self-VERIFIED.)
 */
export const selfUpdateOrganizationSchema = z.object({
  name: z.string().min(2).max(100).optional(),
  email: z.string().email("Must be a valid email address").optional(),
  // Stored as a bare hostname (DNS-TXT verification + similarity checks expect
  // one); accepts a bare hostname or a full URL and normalizes to the hostname.
  domain: domainName().optional(),
  billingEmail: z.string().email("Must be a valid email address").optional(),
});

export type SelfUpdateOrganizationInput = z.infer<typeof selfUpdateOrganizationSchema>;
...
export const locationSchema = z.object({
  ...
  // When true (default), verification fails closed unless the scanner is
  // inside the fence. When false, the binding is advisory — distance is
  // surfaced in `location_match` but never blocks. See
  // packages/api/src/routes/verify.ts for enforcement.
  requireProximity: z.boolean().default(true),
});
...
export const updateQRCodeSchema = z.object({
  ...
  maxScans: z.number().int().positive().optional(),
  maxScansPerDevice: z.number().int().positive().optional(),
  // Toggle geo-fence enforcement on a QR that already has a location set.
  // Server rejects the update when the QR is not geo-bound (lat/lng null).
  requireProximity: z.boolean().optional(),
});
...
export const createAppSchema = z.object({
  ...
  // NOTE: redirectUrls intentionally NOT switched to httpUrl() — native/mobile
  // OAuth clients legitimately register custom-scheme redirect URIs. These are
  // additionally allowlist-checked at use time (see auth-sessions.ts).
...
export const createAuthSessionSchema = z.object({
  // 'cli' is the first-party scope for the qrauth-cli app (ADR-0002). The app's
  // allowedScopes is ['cli'], so without it here the qrauth-cli session-create
  // is unsatisfiable: 'cli' fails this enum, and the default 'identity' fails
  // the app's allowedScopes check — `qrauth login` 400s before approve.
  scopes: z.array(z.enum(['identity', 'email', 'organization', 'cli'])).min(1).default(['identity']),
...
export const proximityAttestationRequestSchema = z.object({
  ...
  /**
   * Caller-supplied device identity (WebAuthn credentialId hash, platform
   * attestation output, or equivalent). The server SHA3-256s this value
   * into the `device` claim (AUDIT-FINDING-006). It is not a hardware
   * attestation — the client-reported caveat still applies.
   */
  deviceFingerprint: z.string().min(1).max(1024),
});
