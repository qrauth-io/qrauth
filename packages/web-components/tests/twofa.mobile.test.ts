...
describe('<qrauth-2fa> mobile-aware pending body', () => {
  ...
  it('opens the hosted approval page in a new tab when the mobile CTA is clicked', async () => {
    ...
    expect(openSpy).toHaveBeenCalledWith(
      'https://test.qrauth.io/a/tok-2fa-123?dr=1',
      '_blank',
      'noopener',
    );
    openSpy.mockRestore();
  ...
  it('falls back to same-tab navigation when window.open returns null (popup blocked)', async () => {
    ...
    expect(hrefSetter).toHaveBeenCalledWith('https://test.qrauth.io/a/tok-2fa-blocked?dr=1');
  });

  it('appends dr=1 exactly once and does not duplicate on re-click', async () => {
    mockMatchMedia(coarseMobileMatch);
    const el = await mountTwoFA();
    (el as unknown as { _sessionToken: string })._sessionToken = 'tok-2fa-once';

    const openSpy = vi.spyOn(window, 'open').mockImplementation(() => ({} as Window));
    const callOpen = (el as unknown as { _openApprovalPage: () => void })._openApprovalPage;
    callOpen.call(el);
    callOpen.call(el);

    expect(openSpy).toHaveBeenCalledTimes(2);
    for (const [href] of openSpy.mock.calls) {
      const u = new URL(href as string);
      expect(u.pathname).toBe('/a/tok-2fa-once');
      expect(u.searchParams.getAll('dr')).toEqual(['1']);
      expect(u.search).toBe('?dr=1');
    }
    openSpy.mockRestore();
  });
});
