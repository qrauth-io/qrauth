...
describe('<qrauth-login> mobile-aware pending body', () => {
  ...
  it('opens the hosted approval page in a new tab when the mobile CTA is clicked', async () => {
    ...
    expect(openSpy).toHaveBeenCalledWith(
      'https://test.qrauth.io/a/tok-abc-123?dr=1',
      '_blank',
      'noopener',
    );
    openSpy.mockRestore();
  ...
  it('falls back to same-tab navigation when window.open returns null (popup blocked)', async () => {
    ...
    expect(hrefSetter).toHaveBeenCalledWith('https://test.qrauth.io/a/tok-blocked?dr=1');
  });

  it('appends dr=1 exactly once and does not duplicate on re-click', async () => {
    mockMatchMedia(coarseMobileMatch);
    const el = await mountLogin();
    (el as unknown as { _sessionToken: string })._sessionToken = 'tok-once';
    ...
    for (const [href] of openSpy.mock.calls) {
      ...
      expect(u.pathname).toBe('/a/tok-once');
